...
def update(i):
	...
	print(label)
	# 更新直线和 x 轴（用一个新的 x 轴的标签）
	# 用元组 Tuple 的形式返回在这一帧要被重新绘图的物体
	line.set_ydata(x - 5 + 0.1 * i)
	ax.set_xlabel(label)
	return line, ax

if __name__ == '__main__':
	# FuncAnimation 会在每一帧都调用 update 函数。
	# 在这里设置一个 30 帧的动画，每帧之间间隔 100 毫秒
	anim = FuncAnimation(fig, update, frames=np.arange(0, 100), interval=70)
